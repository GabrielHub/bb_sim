...
namespace Ball {
	...
	int Team::play(Team* rhs) {
		...
		if (steal(rhs->stlMod)) {
			...
			if (block(rhs->blkMod)) {
				...
				if (rebound(orMod)) {
					tRebound += 1;
					int player = statGiver(1);
					cout << "Offensive Rebound!" << endl;
					return possession;
				}
				else {
					int player = rhs->statGiver(1);
	...
	int Team::statGiver(int statNum) const {
		...
		// Stat for Rebounds
			...
			//int allMax = 0;
			//float minRebound = players[player]->getMaxReb();
			////Check if all players have reached their max avg rebounds
			//for (Player* pp : players) {
			//	if (pp->getMaxReb() == pp->getRebounds()) {
			//		allMax++;
			//	}
			//	//get stats for the lowest rebounder
			//	if (pp->getMaxReb() <= minRebound) {
			//		minRebound = pp->getMaxReb();
			//	}
			//}
			//if (allMax == 5) {
			//	while (players[player]->getMaxReb)
			//}

			//Temporarily just give random player rebound stat;
			players[player]->setRebounds();
			cout << players[player]->getName() << " with the rebound!" << endl;
		}
		//Stat for assists
		else if (statNum == 2) {
		...
		//Stat for Steals
			...
			cout << players[player]->getName() << " steals the ball!" << endl;
			players[player]->setSteals();
		}
		//Stat for Blocks
		else if (statNum == 4) {
			...
			cout << players[player]->getName() << " with a block!" << endl;
			players[player]->setBlocks();
		}
